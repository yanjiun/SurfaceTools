...
import copy
#from surfaceNEW import *
from itertools import permutations
import numpy as np
from gcd import gcd
import pickle

# TO WRITE: 1. compare stored index list with surfaces currently calculated, and#              try to calculate those... 
#
...
def findSizes(indices):
    """
    returns a dictionary of the indices as keys corresponding to size of building an ase atoms class with    size (3,3,10)
    """
    result_dict = {}
    for ind in indices:
        surf = makeSurface('Pt','fcc',ind,size=(3,3,10))
        no_atoms = surf.get_number_of_atoms()
        result_dict[str(ind)] = no_atoms
